...
void Robot::move(Velocity_t *_velocity, Vector_t *_position, Euler_t *_euler, uint32_t time, int step_num) {
  ...
  //Serial.printf("%f %f %f\r\n",_velocity->vx,_velocity->vy,_velocity->omega);
...
void Robot::_avoid(uint16_t dis_now,uint16_t dis_avoid) {
  Serial.println(dis_now);
  Velocity_t vel = {0.0f, 0.0f, 0.0f};
  Vector_t pos = {0.0f, 0.0f, 0.0f};
  Euler_t att = {0.0f, 0.0f, 0.0f};

  if(dis_avoid < 200) dis_avoid = 200;
  ...
  switch(avoid_state) {
    case FORWARD:
      vel = {0.0f, 2.0f, 0.0f};
      move(&vel, &pos, &att, 800);      
      if(dis_now < dis_avoid && dis_now > 100) {
        avoid_state = TURN;
      }
      else if(dis_now < 100) {
        avoid_state = BACK;
    ...
    case WAIT:
      if(_step_num == 0) {
        if(dis_now > dis_avoid) {
          avoid_state = FORWARD;
        }
        else if(dis_now < dis_avoid && dis_now > 100) {
          avoid_state = TURN;
        }
        else if(dis_now < 100) {
          avoid_state = BACK;
...
void Robot::read_position(float *val) {
  val[0] = position.x;
  val[1] = position.y;
  val[2] = position.z;
}
